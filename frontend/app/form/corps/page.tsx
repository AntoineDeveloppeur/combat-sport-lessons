...
export default function Body() {
  ...
  const [instructionCount, setInstructionCount] = useState<number>(
    lesson.bodyInstructions?.length || 1
  )
  const Router = useRouter()

  const validationSchema = Yup.object().shape({
    bodyInstructions: Yup.array()
      .of(
        Yup.object().shape({
          text: Yup.string()
  ...
  const defaultValues = lesson.bodyInstructions
    ? lesson
    : ({
        bodyInstructions: [
          {
    ...
    <Form onSubmit={handleSubmit(saveData)}>
      <FieldSet className="flex flex-col items-start w-[600px]">
        <FieldLegend className="mb-4 text-lg font-semibold">
          Corps de séance: écriver une suite d&apos;instruction composés de
          texte et d&apos;une durée. Vous pouvez ajoutez autant
          d&apos;instructions que vous pouvez. Ajoutez une instruction avec le
          boutton &quot;ajouter une instruction&quot;
        </FieldLegend>
