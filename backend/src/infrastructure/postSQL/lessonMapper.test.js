...
const createMockInstruction = (id, type, order) => ({
  instruction_id: id,
  text: `Texte ${id}`,
  type,
  min: 1,
...
const createMockLesson = (sport = "Boxe") => ({
  lesson_id: "lessonId123",
  title: "Test Lesson",
  sport,
...
const createMockDBResult = (data) => ({ rows: [data] })
const createMockInstructionsDB = (instructions) => ({
  rows: instructions,
})
...
describe("mapMany", () => {
  ...
  it("should handle lesson with only one type of instructions", () => {
    const lessonDB = {
      rows: [
        createLessonRow("1", "Boxe", "Boxe", "userId123", "Jab", "body", 5, 1),
        createLessonRow(
          "1",
          "Boxe",
          "Boxe",
